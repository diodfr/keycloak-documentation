...
In order to configure a first login flow in which :
  ...
  - only users already registered in this realm can login,
...
You could set the also set `Sync Mode` to `force` if you want to update the user profile (Last Name, First Name...) with identity provider attributes.

NOTE: This flow can be used if you want to delegate identity to other identity providers (i.e. github, facebook ...) but you want to manage which users that can login.